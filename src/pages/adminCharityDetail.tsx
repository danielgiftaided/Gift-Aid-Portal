...
interface Submission { id: string; submission_date: string; status: string; hmrc_reference: string | null; amount_claimed: number; number_of_donations: number; tax_year: string; hmrc_status: string; hmrc_response_message: string | null; hmrc_claim_xml: string | null; hmrc_correlation_id: string | null; adjustment_amount: number | null; adjustment_explanation: string | null;
  ...
  hmrc_submission_response_xml: string | null  // SUBMISSION_RESPONSE — the final accepted/rejected result
  ...
  const loadData = async () => {
    try {
      ...
      const subData = await fetchAllRows<Submission>(() =>
        supabase.from('submissions').select('id, submission_date, status, hmrc_reference, amount_claimed, number_of_donations, tax_year, hmrc_status, hmrc_response_message, hmrc_claim_xml, hmrc_correlation_id, adjustment_amount, adjustment_explanation, hmrc_acknowledgement_xml, hmrc_submission_poll_xml, hmrc_submission_response_xml, hmrc_delete_request_xml, hmrc_delete_response_xml, hmrc_data_request_xml, hmrc_data_response_xml').eq('charity_id', id).order('submission_date', { ascending: false })
      )
              ...
              <div className="mt-6 border-t border-gray-100 pt-5">
                  ...
                  <button
                    onClick={async () => {
                      const RECOGNITION_FILES = [
                        ...
                        { name: 'SUBMISSION_RESPONSE.xml',        content: viewingXmlFor.hmrc_submission_response_xml || '', required: true },
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {[
                    ...
                    { label: '4. Submission Response',       key: 'hmrc_submission_response_xml',   note: 'Generated when poll returns final result (accepted/rejected)' },
                    { label: '5. Delete Request',            key: 'hmrc_delete_request_xml',        note: 'Generated when poll cycle completes' },
                    { label: '6. Delete Response',           key: 'hmrc_delete_response_xml',       note: 'Generated when poll cycle completes' },
                    { label: '7. Data Request',              key: 'hmrc_data_request_xml',          note: 'Generated by Send DATA_REQUEST button' },
                    { label: '8. Data Response',             key: 'hmrc_data_response_xml',         note: 'Generated by Send DATA_REQUEST button' },
                  ].map(({ label, key, note }) => {
                ...
                {(['hmrc_acknowledgement_xml','hmrc_submission_poll_xml','hmrc_submission_response_xml','hmrc_delete_request_xml','hmrc_delete_response_xml'] as const).some(k => !(viewingXmlFor as any)[k]) && (
