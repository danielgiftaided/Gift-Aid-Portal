...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    // letters/numbers only (no spaces) - matches your prior rule
    ...
    // Save it
    const { error } = await supabaseAdmin.from("charities").update({ charity_number: charityNumber }).eq("id", charityId);
